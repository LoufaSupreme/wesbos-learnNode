...
function autocomplete(input, latInput, lngInput) {
  ...
  var dropdown = new google.maps.places.Autocomplete(input); // uses google api which is linked in a script tag on layout.pug
...
function loadPlaces(map) {
  ...
  axios__WEBPACK_IMPORTED_MODULE_0___default().get("/api/stores/near?lat=".concat(lat, "&lng=").concat(lng)).then(function (res) {
    var places = res.data;
    if (!places.length) return alert('no places found!'); // initialize boundaries for the map, so that it zooms in but shows all the markers:
    // these bounds need to be adjusted whenever we add a marker

    var bounds = new google.maps.LatLngBounds(); // initialize an info box for each marker

    var infoWindow = new google.maps.InfoWindow(); // create invidiual markers for each store and plot it on the google map:
...
function typeAhead(search) {
  ...
  var searchResults = search.querySelector('.search__results'); // on is shortcut for addEventListener from bling.js
  ...
  searchInput.on('input', function () {
    ...
    searchResults.innerHTML = ""; // get search results by fetching our searchStores API route
    ...
    axios__WEBPACK_IMPORTED_MODULE_0___default().get("/api/search?q=".concat(this.value)).then(function (res) {
      if (res.data.length) {
        ...
        searchResults.innerHTML = dompurify__WEBPACK_IMPORTED_MODULE_1___default().sanitize("<div class=\"search__result\">There are no results for ".concat(_this.value, ".</div>"));
      }
    }).catch(function (err) {
      console.error(err);
    });
  ...
  searchInput.on('keyup', function (e) {
    ...
    var items = search.querySelectorAll('.search__result'); // find the search result that is currently active (i.e. hovered over)

    var activeClass = 'search__result--active';
    var current = search.querySelector(".".concat(activeClass)); // set the next search result to be highlighted based on what key is pressed

    var next; // if user pressed down and there is already a highlighted (a.k.a. active) result:
